...
static unsigned int s_end_ms = -1;
#endif
...
# endif
    ...
    memcpy(snpu_buffer, snpu_input, input_win_length * feat_dim_per_frame * sizeof(short));
    gx_dcache_clean_range((uint32_t *)snpu_buffer, feat_dim_per_frame * input_win_length * sizeof(short));
    ...
    if (1) {
        ...
        cur_context->ctx_index = context->ctx_index + 1;
        unsigned char *pre_snpu_buffer = LvpGetSnpuBuffer(context, context->ctx_index);
        unsigned char *cur_snpu_buffer = LvpGetSnpuBuffer(context, context->ctx_index + 1);
        // printf("%x,%x\n", pre_snpu_buffer, cur_snpu_buffer);
        
        
        LvpCTCModelInitSnpuTask(&snpu_task);

        short *input  = LvpCTCModelGetSnpuFeatsBuffer(pre_snpu_buffer);
# ifdef  CONFIG_LVP_MODEL_USE_RNN
        short *output = LvpCTCModelGetSnpuStateBuffer(cur_snpu_buffer);
# else
        short *output = LvpCTCModelGetSnpuOutBuffer(cur_snpu_buffer);
# endif
