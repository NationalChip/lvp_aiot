...
# endif
    ...
    if (start_mode == GX_WAKEUP_SOURCE_COLD || start_mode == GX_WAKEUP_SOURCE_WDT) {
        ...
        printf(LOG_TAG"Copyright (C) 2001-2023 NationalChip Co., Ltd\n");
        ...
        if (strncmp((const char*)otp, "8003A", strlen("8003A")) == 0) {
            ...
            printf(LOG_TAG"Act_Proct:   [%d Byte]\n", actual_protect);
...
#ifdef CONFIG_LVP_ENABLE_KEYWORD_RECOGNITION
        printf(LOG_TAG"Context Num: [%d]\n",  CONFIG_LVP_CONTEXT_NUM);
        printf(LOG_TAG"Snpu Num:    [%d]\n",  CONFIG_SNPU_BUFFER_NUMBER);
        printf(LOG_TAG"D Window:    [%d]\n",  CONFIG_KWS_MODEL_DECODER_WIN_LENGTH);
        printf(LOG_TAG"D Stride:    [%d]\n",  CONFIG_KWS_MODEL_DECODER_STRIDE_LENGTH);
...
#if (defined CONFIG_ENABLE_PLL_FREQUENCY_60M || defined CONFIG_ENABLE_PLL_FREQUENCY_50M || defined CONFIG_ENABLE_PLL_FREQUENCY_48M)
    int pll_multiple = 2;
#else
    int pll_multiple = 1;
#endif
...
int LvpDynamiciallyAdjustCpuFrequency(CPU_FREQUENCE_LEVEL cpu_frequence_level)
{
    ...
    int pll_multiple = 1;
#endif
    if (cpu_frequence_level == CPU_FREQUENCE_2M) {
        gx_clock_set_div(CLOCK_MODULE_SCPU, 12 * pll_multiple);
    } else if (cpu_frequence_level == CPU_FREQUENCE_4M) {
