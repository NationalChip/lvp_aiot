...
static AUDIO_IN_CTRL s_audio_in_ctrl;
...
int LvpAudioInResetPcmRWIndex(void)
{
    if (s_audio_in_ctrl.pcm_write_index - s_audio_in_ctrl.pcm_read_index >= LvpGetContextNum() - LvpGetContextGap()) {
        s_audio_in_ctrl.pcm_read_index = s_audio_in_ctrl.pcm_write_index - 1;
    }
    return 0;
}
