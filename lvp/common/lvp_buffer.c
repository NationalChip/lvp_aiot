...
unsigned char s_snpu_buffer[CONFIG_SNPU_BUFFER_NUMBER][CONFIG_KWS_SNPU_BUFFER_SIZE/16*16 + 16] ALIGNED_ATTR(16);
#endif
...
#ifdef CONFIG_KWS_SNPU_BUFFER_SIZE
    s_context_header.snpu_buffer      = s_snpu_buffer;
    s_context_header.snpu_buffer_size = sizeof(s_snpu_buffer);
    s_context_header.snpu_buffer_num  = CONFIG_SNPU_BUFFER_NUMBER;
#else
    s_context_header.snpu_buffer      = 0;
    s_context_header.snpu_buffer_size = 0;
    s_context_header.snpu_buffer_num  = 0;
#endif
...
void *LvpGetSnpuBuffer(LVP_CONTEXT *context, unsigned int index)
{
    LVP_CONTEXT_HEADER *ctx_header  = context->ctx_header;
    int idx = index % ctx_header->snpu_buffer_num;
    return &s_snpu_buffer[idx][0];
}
...
void *LvpGetAudioOutBuffer(void)
{
    ...
    extern void *GetCtcDecoderWindow(void);
    void * addr = (void *)GetCtcDecoderWindow();
    return (void *)addr;
#else
# if CONFIG_AUDIO_OUT_BUFFER_SIZE
