...
// unsigned int s_dec_start_ms = 0;
// unsigned int s_dec_end_ms = 0;
...
{
    gx_gpio_set_level(CONFIG_LVP_PLAYER_MUTE_PIN, CONFIG_LVP_PLAYER_MUTE_PIN_LEVEL_STATUS);
...
static void lvp_player_callback(unsigned int saddr, unsigned int eaddr)
{
    ...
    if (player_info.status == PLAYER_STATUS_PLAY && LvpQueueGet(&s_voice_player_frame_queue, (unsigned char *)&play_saddr)) {
...
{
    printf("play finish\n");
    LvpAudioInResetPcmRWIndex();// 因为gxdecoder解码的时候暂停了 snpu，导致 ain 积累了不少音频，而播报过程的音频其实是可以丢弃不需要去做识别。
                                // 所以这里手动将 ain 的读写指针直接手动追上
    lvp_audio_out_exit();
    //恢复计时
    gx_rtc_start_tick();
    if (event_cb)
...
{
    const int vol_to_db[] = {-16, -6, 0, 3, 6, 8, 10, 12, 14, 15, 16};
...
static void lvp_audio_out_init(void)
{
    ...
    memset(LvpGetAudioOutBuffer(),  0, AUDIO_OUT_BUFFER_LEN);
    gx_dcache_clean_range(LvpGetAudioOutBuffer(), AUDIO_OUT_BUFFER_LEN);
    lvp_play_buffer = (unsigned char*)LvpGetAudioOutBuffer();
    ...
    gx_gpio_set_direction(CONFIG_LVP_PLAYER_MUTE_PIN, GX_GPIO_DIRECTION_OUTPUT);
    gx_gpio_set_level(CONFIG_LVP_PLAYER_MUTE_PIN, CONFIG_LVP_PLAYER_MUTE_PIN_LEVEL_STATUS);
    ...
    gx_mdelay(10);
    gx_gpio_set_level(CONFIG_LVP_PLAYER_MUTE_PIN, CONFIG_LVP_PLAYER_MUTE_PIN_LEVEL_STATUS^1);
    gx_mdelay(50);
}
...
static int lvp_audio_play(void)
{
    ...
    if (LvpQueueIsEmpty(&s_voice_player_frame_queue))
        return -1;
    if ((player_info.status == PLAYER_STATUS_PREPARE) && LvpQueueGet(&s_voice_player_frame_queue, (unsigned char *)&play_saddr)) {
        frame.saddr = play_saddr;
        frame.eaddr = frame.saddr + voice_info.dec_frame_size - 1;

        // printf("start_addr :%d %d\n", frame.saddr, frame.eaddr);
...
static int LvpVoicePlayerUpdate(LVP_PLAYER_VOICE *voice_event)
{
    ...
    switch (voice_info.decoder)
    {
        ...
        case TYPE_OPUS:
            {
                unsigned char *tmp_buff = (unsigned char *)LvpGetAudioOutBuffer() + AUDIO_OUT_BUFFER_LEN;
                unsigned int tmp_buffer_size = 0;
                ret = lvp_opus_decoder_init(voice_info.sample_rate, voice_info.channels, tmp_buff,  &tmp_buffer_size);
                printf(LOGTAG"tmp_buffer_size:%d\n", tmp_buffer_size);
            }
            break;
#endif
#ifdef USE_AMR
            ...
            && LvpQueueGetDataNum(&s_voice_player_frame_queue) < (LVP_VOICE_PLAYER_FRAME_COUNT - 2))
    {
        ...
        switch (voice_info.decoder)
        {
            ...
            case TYPE_OPUS:
                ...
                // s_dec_start_ms = gx_get_time_ms();
                lvp_opus_decode(resource_position + voice_info.play_pos, enc_len,
                                (char *)(lvp_play_buffer + decode_offset_count * dec_len), dec_len);
                // s_dec_end_ms = gx_get_time_ms();
                // printf("%dms, enc_len:%d, dec_len:%d\n", s_dec_end_ms - s_dec_start_ms, enc_len, dec_len);
                LvpVoicePlayerPutFrame(0, decode_offset_count, dec_len);
                voice_info.play_pos += enc_len;
                break;
#endif
