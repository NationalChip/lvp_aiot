...
IS_INSIDE_WORK_TREE = $(shell git rev-parse --is-inside-work-tree 2>/dev/null)
ifeq ($(IS_INSIDE_WORK_TREE), true)
...
LIBS           += -lcsky_CK804ef_dsp2_nn -lcsky_dsp -lcsky_dsp
LIBS           += -lm -lc
...
lib_objs        = $(boot_objs) $(dev_objs) $(utility_objs) $(common_objs) $(lvp_objs) $(kws_objs) $(app_objs) $(denoise_objs) $(gxdecoder_objs)
...
MCU_NOR_BIN = output/mcu_nor.bin
MCU_ALL_BIN = output/mcu_all.bin
CUSTOM_STORAGE_SPACE_LEN = $(shell echo $$(($(CONFIG_CUSTOM_STORAGE_SPACE_LEN) * 4096)))
RESOURCE_OFFSET = $(CONFIG_RESOURCE_BIN_ADDRESS_IN_FLASH)
MAX_SIZE_KB = 1020
...
ifeq ($(CONFIG_CUSTOM_STORAGE_SPACE), y)
	@$(eval ZERO_FILL_SIZE := $(CUSTOM_STORAGE_SPACE_LEN))
	@dd if=/dev/zero of=output/temp_zero.bin bs=1 count=$(ZERO_FILL_SIZE)
	@dd if=output/temp_zero.bin of=$(MCU_ALL_BIN) bs=1 seek=$(RESOURCE_OFFSET) conv=notrunc
	@rm output/temp_zero.bin
endif
	...
	@mv $(MCU_ALL_BIN) output/mcu_nor.bin
endif

	@rm -f ./output/lvp.zip
	@cp ./docs/lvp_zip_readme.md output/README.md
	@cp ./docs/GX8003A_module_User\'s\ Guide.pdf output/
	@cp .config output/.config
	@cp ./scripts/download.sh output/download.sh
	@cp ./tools/bootx/bootx output/bootx
ifeq ($(IS_INSIDE_WORK_TREE), true)
